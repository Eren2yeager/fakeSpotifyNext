...
const CustomInputRange = (props) => {
  ...
  const calculateSeekValue = (clientX) => {
    ...
    if (!seekbar || !props.max || isNaN(props.max)) return;
  ...
  useEffect(() => {
    ...
    const handleUp = (e) => {
      ...
      if (isDragging) {
        ...
        setIsDragging(false);
        if (props.isDragingRef != undefined) {
  ...
  const current =
    isDragging && props.localState ? props.localState.localValue : props.value;
...
export default React.memo(CustomInputRange);
