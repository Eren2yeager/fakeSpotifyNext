...
const EditArtistDetailsPopup = ({ artist, open, onClose, onUpdate }) => {
  ...
  const [bio, setGenre] = useState(artist?.bio);
  const [image, setImage] = useState(artist?.image);
  const [preview, setPreview] = useState(artist?.image);
  ...
  const handleSubmit = (e) => {
    ...
    startTransition(async () => {
      // The correct API route is /api/become-artist (not /api/artistDashboard/become-artist)
  ...
  return (
    <Dialog open={open} onClose={onClose}>
      ...
      <h2 className="text-2xl font-bold mb-6  text-white">Edit Artist Details</h2>
      ...
      >
        {/* Image Preview */}
        <label className="group w-45 h-45 bg-neutral-800 rounded-full flex items-center justify-center cursor-pointer overflow-hidden relative">
          <input
            type="file"
            accept="image/*"
            disabled={pending}
        ...
        <div className="flex flex-col w-full flex-1 gap-3">
          ...
          <div>
            <label className="block mb-1 text-sm text-gray-300 mr-auto">Bio</label>
            <input
              type="text"
              value={bio}
...
export default EditArtistDetailsPopup;
