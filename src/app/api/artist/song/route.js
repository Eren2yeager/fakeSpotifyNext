...
export async function POST(req) {
  try {
    ...
    try {
      ...
      const { format } = await mm.parseBuffer(Buffer.from(audioBuffer), audioFile.type);
      duration = formatTime(format.duration); // duration in mm:ss
    } catch (err) {
      return Response.json({ error: "Failed to parse audio metadata" }, { status: 422 });
    }
...
export async function PUT(req) {
  ...
  const body = await req.json(); // { id, name, album, image, duration }
