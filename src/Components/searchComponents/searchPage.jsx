  ...
  return (
    <div className="py-4 text-white space-y-6">
        ...
        <div className="flex flex-col p-2 w-full gap-5">
          ...
          <div className="">
            {results.songs.map((song, idx) => (
              ...
              index={idx}
              item={song}
              context={{ type: "Song", id: song._id, name: song.name }}
              allSongs={[song]}
      ...
      {results?.artists?.length > 0 && (
        <>
          <HorizentalItemsList listItems={results?.artists} heading="Artists"/>
        </>
      )}
      {results?.albums?.length > 0 && (
        <>
          <HorizentalItemsList listItems={results?.albums} heading="Albums"/>
        </>
      )}
    </div>
  );
